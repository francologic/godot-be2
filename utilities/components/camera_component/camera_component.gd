...
func update_camera_height(delta: float, direction: int) -> void:
	if camera_controller_anchor.position.y >= crouch_offset and camera_controller_anchor.position.y <= DEFAULT_HEIGHT:
		camera_controller_anchor.position.y = clampf(camera_controller_anchor.position.y + (crouch_speed * direction) * delta, crouch_offset, DEFAULT_HEIGHT)
